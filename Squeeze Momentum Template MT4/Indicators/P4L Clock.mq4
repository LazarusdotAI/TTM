...
extern bool     ShowSydney                    = false;
extern bool     ShowTokyo                     = false;
extern bool     ShowChina                     = false;
...
extern bool     ShowBerlin                    = false;
extern bool 	 ShowLondon                    = false;
extern bool 	 ShowUTC_GMT                   = false; // FYI, UTC_GMT does not change with Daylight Savings Time. In winter, London=UTC; in summer, London=UTC+1. Also, UTC does not highlight "market" hours.
...
extern bool     ShowLocal                     = false; // FYI, this is your Local computer clock. For accuracy, right-mouse on Clock -> Adjust Date/Time ->Internet Time->Update Now
extern bool     ShowPipSpread                 = true ; 
extern bool     ShowBidPrice                  = false; // Bid price
extern bool     ShowVolume                    = false; // Note, in MT4 "tick" volume is not the same as true transaction-volume (which isn't available in FX)
extern bool     ShowRange                     = true ; // Current bar High-Low in pips.
extern bool     ShowPips2open                 = false; // Current bar Open-to-(current)Close in pips.
extern bool     ShowAvgPeriodRange            = false ; // In pips. Any period, but if THIS period, then the color changes to indicate the range is above/below previous bar.
...
extern int      ADR_Bars                      = 6;     // Usually 1,5,10,20,...   Note: A "trick" value of "1" gives the PREVIOUS (Daily) Bar's range
